import java.util.*;
import java.io.*;
public class BlueBookCalculator{
	static BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
	static StringTokenizer st;
	public static long basetoten(long base, String num) {
		long out = 0,expo = 1;
	...
	public static void main(String[] args) throws IOException{
		int T = readInt();
		for(int i = 0; i<T; i++) {
			long a = basetoten(readLong(), readLine()), b = basetoten(readLong(), readLine());
			char c = readCharacter();
			switch(c) {
			case'-':printform(tentobase(readLong(), a-b));break;
			case'+':printform(tentobase(readLong(), a+b));break;
			case'/':printform(tentobase(readLong(), a/b));break;
			case'*':
			long base = readLong();
